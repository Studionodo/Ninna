...
function renderSuoni() {
  ...
  return `
    <div class="card">
      ...
      ${SOUNDS.map((s) => `<button class="soundrow ${sound.playing === s.id ? "on" : ""}" onclick="NINNA.playSound('${s.id}')">
        <span class="soundtext"><b>${t("sound_" + s.id)}</b><small>${t("sounddesc_" + s.id)}</small></span>
        <span class="soundstate">${sound.playing === s.id ? "◼" : "▶"}</span>
      </button>`).join("")}
  ...
  return cats.map((c) => `
    ...
    ${articles.filter((a) => a.cat === c.id).map((a) => `
      <div class="card artcard" onclick="NINNA.toggleArticle('${a.id}')">
        <div class="card-title">${a.t}<span class="dim">${t("read_min", { n: a.min })}</span></div>
